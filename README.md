...
次年度や別の担当者にシステムを引き継ぐ際の手順。引き継ぎに必要な作業は以下の5項目。

| # | 作業                          | 所要時間 |
|---|-------------------------------|----------|
| 1 | Firebase のアクセス権を付与   | 5分      |
| 2 | Vercel のアクセス権を付与     | 5分      |
| 3 | 管理者権限を付与（Firestore） | 5分      |
| 4 | GitHub のアクセス権を付与     | 5分      |
| 5 | LINE のアクセス権を付与       | 10分     |
...
このシステムで使っているLINEの機能は役割が異なる2つのサービスで構成されており、それぞれ別の管理画面がある。

```
LINE Official Account（公式LINEアカウント本体）
  管理画面：manager.line.biz
  役割：来場者がフォローするLINEアカウント。メッセージ送受信の設定など。

LINE Developers Console（開発者向け設定）
  管理画面：developers.line.biz
  役割：QRスキャン後に開くLIFFアプリの設定、プログラムからメッセージを送るMessaging APIの設定。
```

この2つは連携しているが別の管理画面のため、**両方に引き継ぎ先を追加する必要がある。**

#### 重要：オーナー権限は移譲できない

LINEの仕様上、チャンネルのオーナー権限は別のアカウントに移譲できない。つまり、**作成者の個人LINEアカウントが永続的にオーナーとして残る。**

これによる実害：
- 引き継ぎ先が「管理者」として追加されれば、運用上の操作はすべて行える
- オーナーのLINEアカウントが退会・凍結されると、チャンネルが使えなくなる恐れがある
- **オーナーは在学中はLINEアカウントを維持すること**

完全に引き継ぐ場合（オーナーが卒業するなど）は、次年度担当者が新しいLINEチャンネルを作り直し、コード内のLIFF IDとトークンを更新する必要がある（作業自体は1時間程度）。

---

**① LINE Official Account Manager（公式LINEアカウントの管理者を追加）**

manager.line.biz → 設定 → 権限管理 → 権限の種類を「管理者」に選択 → 「URLを発行」

発行されたURLを引き継ぎ先のLINEに送り、承認してもらう。

> URLは24時間で失効するため、すぐに開いてもらうこと。

**② LINE Developers Console（LIFF・Messaging APIの管理者を追加）**

developers.line.biz → プロバイダー → チャンネル → 「Members」タブ → 引き継ぎ先のLINEアカウントを検索して追加
